    ...
    <div className="donut-shops">
      <div className="info-wrapper">
        ...
        <div className="img-wrapper">
          <img src={donutShop?.image} alt="shop-image" className="donut-shop-image" />
        </div>
        <h3>Location: {donutShop?.location}</h3>
        <h4>Rating: {donutShop?.review}</h4>
        <a href={`https://${donutShop.url}`} className="url">
          {donutShop?.url}
        </a>
      </div>
      ...
      <div>
        {donuts?.map((donut) => (
          <div key={donut?._id}>
            <h1>{donut?.name}</h1>
            <img src={donut?.image} alt="donut-image" className="donut-image" />
            <h4 className="price">Price: {donut?.price}</h4>
            <div className="buttons">
              <button onClick={() => deleteDonut(donut?._id)} className="button1">
                x
              </button>
